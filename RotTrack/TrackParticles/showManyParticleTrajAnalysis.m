function processedManyTrajs = showManyParticleTrajAnalysis(image_label,n_traj_start,n_traj_end,start_frame,tsamp,pixelsize_nm,showVideo,minPointsTraj) 
%
% ========================================
% RotTrack.
...
[NUMERIC0,TXT0,RAW0]=xlsread(trajXlsPath,'params FindTrajectsParticles'); % import the data in the sheet named 'params FindTrajectsParticles'.
pos_subarraySize = find(strcmp(TXT0,'subarray_halfwidth')); % row number corresponding to 'subarray_halfwidth' value.
subarray_halfwidth = RAW0{pos_subarraySize,2};
...
%             set(gca,'nextplot','replacechildren');
            ...
            for k = 1:length(frames_list) % loop through frames in track
                ...
                frame = image_data(frames_list(k)).frame_data; % extract frame data.
                frame = double(frame);
                
                imshow(frame,[],'Border','tight','InitialMagnification',150); % show image scaled between its min and max values ([]).
                hold on;
                
                plot(x_values(k),y_values(k),'x','Color','g','MarkerSize',10) % plot found centre-of-mass positions in green.
