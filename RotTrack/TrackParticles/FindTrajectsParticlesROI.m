function particle_results_ROI = FindTrajectsParticlesROI(image_label,start_frame,end_frame,ROI)
% 
% ========================================
% RotTrack.
...
% ========================================
%
% This function is similar to FindTrajectsParticles.m but it uses a ROI
% instead of excluding regions. It crops the image to the ROI from the
% start and returns coordinates (centre of mass, etc.) for particle
% position relative to the ROI, not to the original full frames.
% Not yet integratable with linkTrajSegmentsParticles.m.
%
% INPUTS:
...
% - ROI: region of interest to use. Enter [] (so that isempty(ROI) = 1) if you
% want to use the full frame. ROI: input a structure as follows.
% Region of interest is given by start coordinates (x_start, y_start) for
...
% box on image (only one rectangle can be defined). Example ROI structure:
% ROI.xstart = 1;
% ROI.xend = 820;
% ROI.ystart = 581 ;
% ROI.yend = 614;
% 
% -------------------------------------------------------
% IMPORTANT: this function requires the video file labelled by
% 'image_label'. Before running this function you should move into the directory which
% contains the video file. 
%
% Example of how to call this function: 
% for frames 1 to 10 of image "0508_C3_001.avi" in current folder:
% results1 = FindTrajectsParticlesROI('0508_C3_001',1,'end',ROI); % frame 1 to
% final frame.
% results2 = FindTrajectsParticlesROI('0508_C3_001',1,10,ROI); % frame 1 to 10.
%
%
...
% Results are saved in the structure array particle_results_ROI.
...
%
% OUTPUT:
% The output, particle_results_ROI is a cell array with two elements:
% particle_results_ROI = {params, particle_final};
% The first element in the cell array, particle_results_ROI{1}, contains all parameters used to run
% the function: "params" (this is a structure array itself).
% The second element in the cell array, particle_results_ROI{2}, contains the track
...
%
% Example of params: particle_results_ROI{1}:
%
%             image_label: '25'
%             start_frame: 1
%               end_frame: 10
...
[numFrames frame_Ysize frame_Xsize image_data0 image_path] = extract_image_sequence_data(image_label);
% See "extract_image_sequence_data.m".
% numFrames is the number of frames in the image sequence.
% To get frame number "p" do: image_data0(p).frame_data.
% Frame dimensions are frame_Ysize and frame_Xsize.
% --------------------------------------------------------------

% Crop original image frames (image_data0) to region of interest and save
% to image_data.
emptystruct.frame_data = [];
image_data = repmat(emptystruct,1,numFrames); % initialise empty structure for speed.
for i = 1:numFrames
    frame0 = image_data0(i).frame_data;
    cropped_frame = frame0(ROI.ystart:ROI.yend,ROI.xstart:ROI.xend);
    image_data(i).frame_data = cropped_frame;
end
...
% length containing the x and y coordinates of the candidate particle
% positions found on the image. They contain integer numbers: coordinates
% or pixel numbers which give position on image plane.
...
%% OUTPUT OF particle-FINDING PROCESS: final output particle_results_ROI:
%
particle_results_ROI = {params, particle_final};
