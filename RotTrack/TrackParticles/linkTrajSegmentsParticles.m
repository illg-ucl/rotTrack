function linkTrajSegmentsParticles(image_label,start_frame,end_frame,particle_results,data_set_label)
% 
% ========================================
% RotTrack.
...
% ========================================
%
% This function links and groups all found trajectory segments (for particles) 
...
% linkTrajSegmentsParticles('490',100,110,s1,'ATPase-GFP'),
% where s1 =FindTrajectsParticles('490',100,110) has been previously calculated;
% Takes image sequence containing '490' on its name, within the current folder,
...
% dir_data = 'C:\Users\isabel\ISABEL\UCL_Isabel\Projects\Collaborations\SoniaContera\Andrea_rotatingMagneticParticles\';
...
% For linking end particle position in one trajectory with start particle position in another trajectory:
d_01_max = 30; % max distance in pixels between particle centres.(5)
...
%% Extract segment (small trajectory) results from "particle_results" input:

params_for_FindTrajects = particle_results{1}; % parameters used in function FindTrajects.m to obtain segments.
traj_results = particle_results{2}; % Results of segments, obtained by function FindTrajects.m.

% Remove fields from traj_results structure that contain vectors as opposed
% to single numbers (this is necessary for concatenation with right
% dimensions later on):
fieldsToRemove = {'xpoints_ellipse','ypoints_ellipse','xpoints_majorAxis','ypoints_majorAxis'};
traj_results = rmfield(traj_results,fieldsToRemove);
...
% data arranged in a large matrix (particle after particle).

max_no_particles = size(traj_results,2); % max. number of found particle centres in each frame.

data_to_export_labels = fieldnames(traj_results)'; % Row vector with the fieldnames of all found particles.
% ('estimateXcentre','estimateYcentre','Xcom','Ycom','AngleDegrees','majorAxisLength','minorAxisLength','ClipFlag','TooCloseToEdge','FrameNumber','ParticleNumber','TrajNumber').

% Make of class single to save memory space:
all_data_to_export = single([]); % empty, to be filled in (original data, all particles).
data_to_export = single([]); % empty, to be filled in (only particles that have been linked into trajectories will be saved to this variable).
...
x_column = strmatch('Xcom',data_to_export_labels); % number of column containing x centre-of-mass position (used later).
y_column = strmatch('Ycom',data_to_export_labels); % number of column containing y centre-of-mass position (used later).
...
%             plot(traj_results(k,q).Xcom,traj_results(k,q).Ycom,'o','Color',color_list(tr,:),'MarkerSize',10)
            % Construct trajectory data to save (convert structure to matrix):
            data_to_export = [data_to_export; single(cell2mat(struct2cell(traj_results(k,q)))')]; % append row with the particle data.
        end
    end
%     pause(0.2);
%     hold off;
...
% Sort particle data by trajectory number and separate trajectories:
...
% Sort particles data by trajectory number:
if ~isempty(data_to_export)
    ...
    disp('!!')
    disp('WARNING: particles found were not linked into trajectories. No trajectories of more than one point were found.')
    disp('No excel file of trajectory results was saved.')
    disp('!!')
    return
end
...
% Xcom values (1st column) of trajectory number "tr".
% Note that the first row in each trajectory is reserved for adding column
% labels latter!!!
%--------------------------------


% Error control: if there is only one trajectory number (traj number 1 or 0 for loose particles):
if unique([traj_results.TrajNumber])<=1
    ...
    output_filename = strcat(data_set_label,'_',image_label,'allparticles.xls'); % output .xls filename (before sorting data by trajectory number)
...
%         end
        ...
        y_end = separated_trajs{qend}(end,y_column); % Ycom position for last particle in trajectory qend.
        Frame_end = separated_trajs{qend}(end,Frame_column); % frame number for last particle in trajectory qend.
        
        x_start = separated_trajs{qstart}(1,x_column);   % Xcom position for first particle in trajectory qstart.
        y_start = separated_trajs{qstart}(1,y_column);   % Ycom position for first particle in trajectory qstart.
        Frame_start = separated_trajs{qstart}(1,Frame_column); % frame number for first particle in trajectory qstart.
        
        % d_01: distance between end particle in trajectory qend and start point in trajectory qstart:
        d_01 = sqrt((x_end-x_start)^2+(y_end-y_start)^2);
        % Distance in frames (proportional to time) between end particle in trajectory qend and start point in trajectory qstart:
        Frames_away = Frame_start-Frame_end; % should be >0 for linking segments (see later).
        
        %                         d_01
        %                         Frames_away
        
        % Accept asignment (link trajectory segments) and save values only if start and end particles in
...
% --------------------------------------------------------
% GROUP BEST ASSIGNMENTS WHICH CORRESPOND TO THE SAME particle:
...
% Different trajectories are plotted as different colour circles.
...
%     k
    ...
    for q = 1:max_no_particles % loop through found particles on each frame:
        ...
        % full_traj_results(k,q) is a structure containing the found particle
        % characteristics, including fields 'Xcom', 'Ycom' and
        % 'TrajNumber'.
        tr = traj_results(k,q).TrajNumber; % trajectory number corresponding to found particle q on frame k (in old traj results).
        
        %         tr
        
        if isempty(old_new_traj_numbers)==0 % if there are traj numbers to be changed:
            if isempty(tr) == 0 % if that particle is not empty (due to padding of structure array):
                pos_to_change = find(old_new_traj_numbers(:,1)==tr); % check if traj number tr for this particle is contained in the list of traj numbers that need to be changed (old_new_traj_numbers).
                
                %             pos_to_change
                
                if isempty(pos_to_change) == 0 % if the traj number tr for this particle is contained in the list of traj numbers that need to be changed:
        ...
        if  tr > 0  % only plot trajectory numbers > 0 (TrajNumber = 0 is for loose particles):
            % Plot each trajectory in a different colour:
            plot(full_traj_results(k,q).Xcom,full_traj_results(k,q).Ycom,'x','Color',color_list(full_traj_results(k,q).TrajNumber,:),'MarkerSize',5)
            
            % Plot major axis of ellipse to indicate particle orientation:
            xpointsMajorAxis = [
                full_traj_results(k,q).Xcom - 0.5*full_traj_results(k,q).majorAxisLength*cos(full_traj_results(k,q).AngleDegrees/180*pi) 
                full_traj_results(k,q).Xcom 
                full_traj_results(k,q).Xcom + 0.5*full_traj_results(k,q).majorAxisLength*cos(full_traj_results(k,q).AngleDegrees/180*pi)
            ];
            ypointsMajorAxis = [
                full_traj_results(k,q).Ycom + 0.5*full_traj_results(k,q).majorAxisLength*sin(full_traj_results(k,q).AngleDegrees/180*pi) 
                full_traj_results(k,q).Ycom 
                full_traj_results(k,q).Ycom - 0.5*full_traj_results(k,q).majorAxisLength*sin(full_traj_results(k,q).AngleDegrees/180*pi)
            ];
            plot(xpointsMajorAxis,ypointsMajorAxis,'y','LineWidth',1);
            %             % Construct trajectory data to save (convert structure to matrix):
            %             data_to_export = [data_to_export; cell2mat(struct2cell(traj_results(k,q)))']; % append row with the particle data.
        end
    end
    pause(0.1);
    hold off;
...
% % Export also all (non-empty) particles in the input (traj_results) structure, 
