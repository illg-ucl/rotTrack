...
minPointsTraj = 10;
% time between frames (tsamp) is inverse of frame rate:
frameRate = 30; % frame rate (frames per second).
tsamp = 1/frameRate; % time between frames in seconds.
...
for i = 1:length(videoLabel)   
   ...
   tracks{i} = FindTrajectsParticles(videoLabel{i},1,'end',[]);
   ...
   plotParticleTrajNumbers(videoLabel{i},data_set_label,minPointsTraj); 
   cd(data_folder);
...
for i = 1:length(videoLabel)   
   ...
   processedTrajs{i} = showManyParticleTrajAnalysis(videoLabel{i},data_set_label,1,'end',1,tsamp,1,1,1,minPointsTraj);
