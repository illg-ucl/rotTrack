function [xx,yy] = findCandidateParticlePositions(frame,method)
%
% ========================================
% RotTrack.
...
% ========================================
%
% Find candidate x, y positions of particles on images. The function
% recognises dark particles. Two methods are used: (1) morphological operations to obtain
...
% is far more sensitive to uneven background illumination and nearby particles.
% None of these methods give sub-pixel precision, though.
%
% The candidate positions can later be used as input of function
% "findParticleCentre1frame".
%
% INPUTS:
...
% - method: 1 for  morphological operations to obtain candidates for particle 
%   centres (thresholding, eroding, etc.); 
%           2 for centre-of-mass method;
%           0 to plot and compare both methods (output is that of Method 1);
%
% OUTPUTS:
% Candidate particle-centre positions obtained from chosen method:
% - xx: columns of positions of candidates (horizontal).
% - yy: rows of positions of candidates (vertical).
%
%
...
threshold_factor = 1; % For thresholding, aiming to detect the black centre of particles. Default 0.64.
...
% A factor ~0.4 times the automatic threshold helps separate nearby particles
% but does not allow the detection of dimmer particles. 
aroundBead = 35; % For centre of mass method: halfsize of subarray (in pixels) taken around each particle. Default 35-50.
...
% This eliminates small pixel areas which are not actually particles:
% SignalMask2 = imopen(SignalMask,se2); 
SignalMask2 = SignalMask;

% Reduce particles to single points:
...
% In principle, sum(sum(signalMask4)) is the number of particles in the image.
...
% The positions of the candidate particle positions are:
...
if method == 2 || method == 0
    % Use CENTRE OF MASS METHOD to obtain slightly more precise particle centres
    % (though still not with sub-pixel precision).
    
    % Halfsize of subarray (in pixels) taken around each particle candidate
    ...
    for i = 1:length(xx0)
        ...
        xx_0 = xx0(i);
        yy_0 = yy0(i);
        % If the particle candidate is at the edge of the image move away from the edge
        ...
        % centroid estimate pixel (xx(i),yy(i)). I is fixed during the iterative process of finding the centre of the particle.
        ...
        % Calculate revised estimates of x and y particle centre position by centre
